...
import 'package:lifeschool/explore/mastermind_detail_widget.dart';
...
class MastermindWidgetState extends State<MastermindWidget> {
  ...
  Widget build(BuildContext context) {
    void _handleOnTap() {
      Navigator.push(
        context,
        MaterialPageRoute(
            builder: (context) => MastermindDetailWidget(widget.mastermind)),
      );
    }
      ...
      children: <Widget>[
        CarouselSlider(
          items: widget.mastermind.imageUrls.map((url) {
            return Hero(
                tag: 'mastermind_carousel_' +
                    url.toString(), // TODO: animate widget to work across screens
                child: Image.asset(
                  url,
                  fit: BoxFit.fill,
  ...
  Widget _buildTitle() {
    return Container(
        padding: EdgeInsets.only(top: 2.0, left: 12.0, right: 12.0),
